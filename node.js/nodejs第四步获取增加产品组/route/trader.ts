 ...
 * 获取指定商家的所有的产品组 
 */
traderRouter.all('/getAllGroups', async (req, res, next) => {
    var result = new Result();
    //商家的用户名
    ...
    if (username) {
        ...
        if (trader) {
            var productGroups = await TraderSevice.getAllGroupByTraderID(trader._id);
            result.issucess = true;
...
//商家增加促销产品
traderRouter.all('/addSale', async (req, res, next) => {
    var result = new Result();
    var {username, banner, url} = req.query;
    if (username && banner && url) {
        var trader: Trader = await TraderSevice.getTraderByName(username);
        if (trader) {
            var data = await TraderSevice.addSales(trader._id, url, banner);
            result.issucess = true;
            result.data = data;
...
traderRouter.all('/getAllSales', async (req, res, next) => {
    var result = new Result();
    var {username} = req.query;
    var trader: Trader = await TraderSevice.getTraderByName(username);
    var sales = await TraderSevice.getSales(trader._id);
    result.data = sales;
